...
- **`public/manifest.webmanifest`** — name, `theme_color`/`background_color` (`#0c0705`, the dark board), `display: standalone`, and three PNG icons (192/512 `any` + 512 `maskable`). `index.html` has a single `<meta name="theme-color">` whose `content` is swapped in JS (`applyThemeSideEffects`) between paper `#fcf7ef` (light) and board `#0c0705` (dark) to follow the active theme.
