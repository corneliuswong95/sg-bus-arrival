...
- **Hero board** (`#next-board` / `renderNextBoard`): the soonest-arriving service for the stop, rendered as an amber LED departure-board readout at the top of the sheet — route bullet + destination blind + dotted leader + big Doto countdown, with a crowding/`then` sub-line. Hidden when there are no timings; hidden in `route-mode`.
...
> Design history: the app was originally a Material-red, system-font UI; the "Board" cyberpunk redesign landed on branch `redesign/cyberpunk-board`. If a color/font looks "off", check it against the token system above, not the old Material palette.
